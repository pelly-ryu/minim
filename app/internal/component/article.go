	...
	return app.Div().ID("article").Body(
		app.Div().Class("article-header pure-g").Body(
			app.Div().Class("pure-u-1-2").Body(
				app.H1().Class("article-title").ContentEditable(true).Text("Hello from Toronto"),
				app.P().Class("article-subtitle").Body(
					app.Span().Text("3:56pm, April 3, 2012"),
				),
			),
			app.Div().Class("article-controls pure-u-1-2").Body(
		...
		app.Div().Class("article-body").ContentEditable(true).Body(
